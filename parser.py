...
class Parser:
    def __init__(self, agent):
        self.agent = agent
        self.genes_to_parse = deepcopy(self.agent.curr_genome.genes)
        self.children = []
        self.depth = 0
    ...
    def parse_node(self):
        if len(self.genes_to_parse) == 0:
            return Do_Nothing(agent=self.agent)
        self.depth += 1
        if self.depth >= 100:
            return Do_Nothing(agent=self.agent)
        val = self.genes_to_parse.pop(0) % 3
        if val == 0:
    ...
    def parse_children(self):
        if len(self.genes_to_parse) == 0:
            return Do_Nothing(agent=self.agent)
        val = self.genes_to_parse.pop(0) % 2
        # if val == 0:
